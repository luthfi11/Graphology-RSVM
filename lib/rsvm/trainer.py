...
import lib.rsvm.split as split
import lib.rsvm.kernel as kernel
import lib.rsvm.SSVM as SSVM
...
class Trainer():
    def __init__(self, data, label_pos):
        ...
        try:
            numpy.save('lib/rsvm/rawData', data )
            numpy.save('lib/rsvm/tempLabel',self.label)
            numpy.save('lib/rsvm/tempData',self.data)
        except:
            print("\n===Error in trainer-init : can't save temp data===")

        self.rawData = 'lib/rsvm/rawData.npy'
        self.tempLabel = 'lib/rsvm/tempLabel.npy'
        self.tempData = 'lib/rsvm/tempData.npy'
...
###############################################################################
    ...
    def make(self, r=1, v=1):
        ...
        if 'self.label' not in locals() or 'self.data' not in locals():
            try:
                self.label = numpy.load(self.tempLabel, allow_pickle = True)
                self.data = numpy.load(self.tempData, allow_pickle = True)
            except:
    ...
    def multiClassifier(self, ith):
        ...
        if self.strategy == 0:
            if K['flag'] == 'dual':
                for i in label_var:
                    model[i] = {}
                    for j in label_var[numpy.where(label_var == i)[0][0] + 1 : label_var.shape[0]]:
                        model[i][j] = {}
